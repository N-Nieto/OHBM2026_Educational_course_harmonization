...
def simulate_batched_data2(
    ...
    """
    Simulate batched data from a baseline outcome vector or matrix.

    Returns
    -------
    dict with:
        - "df": pandas.DataFrame with batch, covariates, and component terms
        - "y": numpy.ndarray with shape (n, n_cols)
        - "y_base": numpy.ndarray with shape (n, n_cols)
        - "y_covariate": numpy.ndarray with shape (n, n_cols)
        - "batch_add": numpy.ndarray with shape (n, n_cols)
        - "noise_scale": numpy.ndarray with shape (n, n_cols)
        - "noise": numpy.ndarray with shape (n, n_cols)
    """
    rng = np.random.default_rng(seed)
    ...
    if data.ndim == 1:
        y_base = data.reshape(-1, 1)
        if n_cols > 1:
            y_base = np.repeat(y_base, n_cols, axis=1)
    elif data.ndim == 2:
        y_base = data.copy()
        if y_base.shape[1] != n_cols:
            raise ValueError(
                f"`data` has {y_base.shape[1]} columns, but `n_cols={n_cols}`."
            )
    else:
    ...
    if y_base.shape[0] != batch.shape[0]:
        raise ValueError(
            f"`data` and `batch` must have the same number of rows. "
            f"Got {y_base.shape[0]} and {batch.shape[0]}."
        )
    ...
    # component arrays
    y_covariate = np.zeros((n, p), dtype=float)
    batch_add = np.zeros((n, p), dtype=float)
    noise_scale = np.ones((n, p), dtype=float)
    noise = np.zeros((n, p), dtype=float)
    ...
    for b in batch_levels:
        ...
        for cov_name, spec in covariate_specs.items():
            ...
            if dist == "normal":
                ...
                p_cov = resolve(spec.get("p", 0.5), b, 0.5)
                p_cov = float(p_cov)
                if not (0.0 <= p_cov <= 1.0):
                    raise ValueError(
                        f"Bernoulli probability for '{cov_name}' in '{b}' must be between 0 and 1."
                    )
                x = rng.binomial(n=1, p=p_cov, size=n_b)
            ...
            df.loc[mask, cov_name] = x
            beta = float(betas.get(cov_name, 0.0))
            cov_effect += beta * x

        pparams = batch_params.get(b, {})
        add_mean = float(pparams.get("add_mean", 0.0))
        add_sd = float(pparams.get("add_sd", 0.0))
        ...
        for j in range(p):
            batch_add[idx, j] = rng.normal(loc=add_mean, scale=add_sd, size=n_b)

            multi_mean = float(pparams.get("multi_mean", 1.0))
            multi_shape = float(pparams.get("multi_shape", 20.0))
            ...
            invgamma_scale = multi_mean * (multi_shape - 1.0)
            gamma_draw = rng.gamma(
                shape=multi_shape, scale=1.0 / invgamma_scale, size=n_b
            )
            noise_scale[idx, j] = 1.0 / gamma_draw

            eps = rng.normal(loc=0.0, scale=noise_sd, size=n_b)
            noise[idx, j] = eps * noise_scale[idx, j]

            y_covariate[idx, j] = cov_effect

    y = y_base + y_covariate + batch_add + noise

    # store scalar/summary versions in df
    if p == 1:
        df["y_base"] = y_base[:, 0]
        df["y_covariate"] = y_covariate[:, 0]
        df["batch_add"] = batch_add[:, 0]
        df["noise_scale"] = noise_scale[:, 0]
        df["noise"] = noise[:, 0]
    else:
        df["y_base_mean"] = y_base.mean(axis=1)
        df["y_covariate_mean"] = y_covariate.mean(axis=1)
        df["batch_add_mean"] = batch_add.mean(axis=1)
        df["noise_scale_mean"] = noise_scale.mean(axis=1)
        df["noise_mean"] = noise.mean(axis=1)

    return {
        "df": df,
        "y": y,
        "y_base": y_base,
        "y_covariate": y_covariate,
        "batch_add": batch_add,
