...
def build_style_registry(
    ...
    idp_style = {
        i_name: (idp_colors[i % len(idp_colors)], idp_markers[i % len(idp_markers)])
...
def plot_WithinSubjVar(
    ...
    subject_style, idp_style = build_style_registry(
        subjects, idps, subject_palette="tab10", idp_palette="Set2"
    )
    ...
    if subject_style is None or idp_style is None:
        raise ValueError(
            "Provide subject_style and idp_style from build_style_registry()"
        )
    ...
    long = df.melt(
        id_vars=subject_col, value_vars=idps, var_name="IDP", value_name="value"
    )
    ...
    for idp, mean_val in idp_means.items():
        if show_idp_legend:
            ...
            axB.scatter(
                mean_val, 0, s=90, marker=marker, color=color, edgecolor="k", label=idp
            )
        else:
            axB.scatter(mean_val, 0, s=70, marker="o", color="gray", edgecolor="k")
    ...
    for subj, mean_val in subj_means.items():
        if show_subject_legend:
            color, marker = subject_style[subj]
            axC.scatter(
                mean_val, 0, s=90, marker=marker, color=color, edgecolor="k", label=subj
            )
        else:
            axC.scatter(mean_val, 0, s=70, marker="o", color="gray", edgecolor="k")
...
def plot_SubjectOrder_old(
    ...
    stacked = np.stack(
        [rho_mats_all[idp].values for idp in all_idps], axis=0
    )  # shape (n_idps, n_times, n_times)
    stacked_p = np.stack([p_mats_all[idp].values for idp in all_idps], axis=0)
    ...
    combined_p_matrix = np.full((n_times, n_times), np.nan)
    combined_rho_matrix = np.nanmean(
        stacked, axis=0
    )  # keep mean rho (for sign in Stouffer)
    if combine_method.lower() == "fisher":
        ...
        for i in range(n_times):
            for j in range(n_times):
                ...
                if pv.size == 0:
                    ...
                    zs = norm.ppf(
                        1.0 - p_clip / 2.0
                    )  # two-sided -> two-tailed z magnitude
                    # apply sign
                    signed_zs = sign_cell * zs
    ...
    if combine_method.lower() == "fisher":
        ...
        for idp in all_idps:
            rho = rho_mats_all[idp].values
            mask = ~np.eye(n_times, dtype=bool)
            idp_mean_rhos.append(
                np.nanmean(rho[mask]) if ~np.all(np.isnan(rho[mask])) else np.nan
            )
    else:
...
def plot_additive_multiplicative_effects(df, feature_cols=None, batch_col="Site"):
    if feature_cols is None:
        feature_cols = [
            c for c in df.columns if c not in ["Subject", "Timepoint", batch_col]
        ]
    ...
    for i, feat in enumerate(feature_cols):
        ...
        data_by_site = [
            df.loc[df[batch_col] == site, feat].dropna().values for site in sites
        ]

        ax.boxplot(data_by_site, tick_labels=sites, showmeans=True)
        ax.set_title(
            f"{feat}: additive shift shows as center change, multiplicative effect as spread change"
        )
