...
def WithinSubjVar_long(
    ...
    if len(timepoints) != n_samples:
        raise ValueError(
            "Length of timepoints must match number of rows in idp_matrix."
        )
    ...
    for ia in range(len(tp_levels) - 1):
        for ib in range(ia + 1, len(tp_levels)):
            ...
            for j, idp_name in enumerate(idp_names):
                xa_raw = (
                    idp_matrix[idxA, j] if j < n_idps else np.array([], dtype=float)
                )
                yb_raw = (
                    idp_matrix[idxB, j] if j < n_idps else np.array([], dtype=float)
                )
                ...
                null_mean = (
                    float(sum_null / valid_null_count)
                    if valid_null_count > 0
                    else np.nan
                )
                pval = (
                    float((1 + count_ge) / (1 + valid_null_count))
                    if valid_null_count > 0
                    else float("nan")
                )
                ...
                rows.append(
                    {
                        ...
                        "IDP": idp_name,
                        "nPairs": int(nPairs),
                        "SpearmanRho": float(obs_rho)
                        if not np.isnan(obs_rho)
                        else np.nan,
...
# -------------------------
def _build_fixed_formula_terms(
    fix_eff: Sequence[str], data: pd.DataFrame, do_zscore_predictors: bool = True
) -> List[str]:
    """
...
# -------------------------
def _safe_fit_mixedlm(
    formula_fixed: str, data: pd.DataFrame, group: str, reml: bool = False
):
    ...
    if covariates:
        for cname, seq in covariates.items():
            if cname in df.columns:
                warnings.warn(
                    f"Covariate '{cname}' already exists in DataFrame; skipping insertion."
                )
                continue
            seq = list(seq)
            if len(seq) != len(df):
                raise ValueError(
                    f"Length of covariate '{cname}' ({len(seq)}) does not match number of rows ({len(df)})."
                )
...
def AdditiveEffect_long(
    ...
    *,
    ...
    if missing:
        raise KeyError(
            f"Variables not found in data columns: {missing}. Available columns: {list(df.columns)}"
        )
    ...
    for idx, feat in enumerate(feature_cols, 1):
        if verbose:
            print(
                f"[AdditiveEffect_long] ({idx}/{V}) testing additive batch effect for feature: {feat}"
            )
        ...
        fixed_terms = _build_fixed_formula_terms(
            list(fix_eff or []), local_df, do_zscore_predictors=True
        )
        missing_fix = [
            vv
            for vv in (fix_eff or [])
            if (f"zscore_{vv}" not in local_df.columns) and (vv not in local_df.columns)
        ]
        if missing_fix:
            warnings.warn(
                f"Fixed-effect columns not found in data (or zscore missing): {missing_fix}"
            )
        ...
        try:
            res_full = _safe_fit_mixedlm(
                full_fixed, local_df, group=group_name, reml=reml
            )
        except Exception as e:
            if verbose:
                print(f"  full fit failed for {feat}: {e}")
        try:
            res_red = _safe_fit_mixedlm(
                reduced_fixed, local_df, group=group_name, reml=reml
            )
        except Exception as e:
            if verbose:
        ...
        if not np.isfinite(pval) and (res_full is not None):
            try:
                ...
                for i, pn in enumerate(pnames):
                    if (
                        (f"C({batchcol})" in pn)
                        or (f"{batchcol}[T." in pn)
                        or pn.startswith(f"{batchcol}_")
                    ):
                ...
                if len(batch_param_indices) == 0:
                    ...
                    for i, pn in enumerate(pnames):
                        for lvl in cats:
                            if f"{lvl}" in str(pn) and (
                                batchcol in pn or f"C({batchcol})" in pn
                            ):
                                batch_param_indices.append(i)
                                break
                if len(batch_param_indices) > 0:
                    ...
                    cov_mat = (
                        cov.to_numpy() if hasattr(cov, "to_numpy") else np.asarray(cov)
                    )
...
def MultiplicativeEffect_long(
    ...
    *,
    ...
    if isinstance(helper_out, tuple) and len(helper_out) >= 1:
        ...
        try:
            ...
            idcol = (
                helper_out[1]
                if len(helper_out) > 1
                else (idvar if idvar is not None else "subjects")
            )
            batchcol = (
                helper_out[2]
                if len(helper_out) > 2
                else (batchvar if batchvar is not None else "batches")
            )
            tpcol = (
                helper_out[3]
                if len(helper_out) > 3
                else (timevar if timevar is not None else "timepoints")
            )
        except Exception as e:
            raise RuntimeError(
                f"_build_input_df_if_needed returned an unexpected tuple shape: {e}"
            )
        ...
        if verbose:
            warnings.warn(
                "Detected old _build_input_df_if_needed signature (returned DataFrame). Falling back to conventional column names."
            )
    else:
        raise RuntimeError(
            f"_build_input_df_if_needed returned unsupported type: {type(helper_out)}"
        )
    ...
    for idx, feat in enumerate(feature_cols, 1):
        if verbose:
            print(
                f"[MultEffect_long] ({idx}/{V}) testing multiplicative batch effect for feature: {feat}"
            )
        ...
        try:
            ...
            resid = pd.Series(
                np.asarray(getattr(res_full, "resid", np.asarray([]))),
                index=local_df.index
                if len(local_df) == len(getattr(res_full, "resid", []))
                else None,
            )
        ...
        try:
            ...
            if len(groups) < 2 or any(len(g) == 0 for g in groups):
                raise ValueError(
                    "Not enough observations per batch group for Fligner test."
                )
...
def simulate_longitudinal_batch_data_mixed(
    ...
    slopes = {feat: np.random.normal(0, slope_sd, n_subjects) for feat in feature_names}

    age_effects = {feat: np.random.uniform(-4, 4) for feat in feature_names}
...
def simulate_cross_sectional_harmonization_data(
    ...
    for i in range(n_subjects):
        ...
        for j, feat in enumerate(feature_names):
            ...
            true_value = (
                feature_means[j] + subject_baselines[i, j] + age_slopes[j] * age[i]
            )
        ...
        model_rows.append(
            {
                "Feature": feat,
                "Formula": formula,
                "Site_pvalue": fit.pvalues[
                    [c for c in fit.pvalues.index if "C(Site)" in c]
                ].min()
                if any("C(Site)" in c for c in fit.pvalues.index)
                else np.nan,
                "R2": fit.rsquared,
            }
        )
